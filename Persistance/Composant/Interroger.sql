...
SELECT nom,titre,salaire FROM employe WHERE titre ="Représentant" OR "Président"
...
SELECT salaire,titre,nom FROM employe ORDER BY titre ASC,salaire DESC;
...
SELECT nom,salaire,tauxcom,(tauxcom*salaire) FROM employe WHERE tauxcom IS NOT NULL;
...
tables utilisées).*/
SELECT nodep AS "numero de departement",dept.nom AS " nom de departement",employe.nom AS"nom de l'employé" FROM dept JOIN employe ON dept.nodept= employe.nodep ORDER BY nodept;
...
SELECT e.nom, e.prenom, e.salaire, p.nom, p.prenom, p.salaire
FROM employe AS e
...
FROM employe 
WHERE titre 
IN (SELECT titre FROM employe WHERE nom
Like "Amartakaldire");
...
WHERE nodep = 31)
AND nodep != 31
order BY nodep DESC ,salaire desc
;
...
SELECT ( MAX(salaire)-MIN(salaire)) AS "ecart" ,MAX(salaire),MIN(salaire) FROM employe ;
...
SELECT dept.nom, COUNT(noemp) FROM dept LEFT JOIN employe on dept.nodept= employe.nodep group BY dept.nom;
